...
export default function Game() {
  ...
  useEffect(() => {
    const onTilt = (e: DeviceOrientationEvent) => {
      ...
      gsRef.current.tiltX = Math.max(-1, Math.min(1, e.gamma / 30));
    };
  ...
  useEffect(() => {
    ...
    function drawPlatform(p: Platform) {
      ...
      if (p.type === "spring") {
        ...
        ctx.fillStyle = g;
        ctx.beginPath(); ctx.roundRect(-p.w / 2, -p.h / 2, p.w, p.h, 7); ctx.fill();
        ...
        for (let i = 0; i < 3; i++) {
          ...
          ctx.beginPath(); ctx.moveTo(-8, cy); ctx.quadraticCurveTo(0, cy - 6, 8, cy); ctx.stroke();
        }
        ctx.strokeStyle = "#e8305a"; ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(-6, -p.h / 2 - coilH); ctx.lineTo(-6, -p.h / 2);
        ctx.moveTo(6, -p.h / 2 - coilH); ctx.lineTo(6, -p.h / 2);
        ctx.stroke();
        ...
        if (p.cracked) {
          ...
          ctx.beginPath();
          ctx.moveTo(-p.w / 4, -p.h / 2); ctx.lineTo(-p.w / 6, p.h / 2);
          ctx.moveTo(p.w / 5, -p.h / 2 + 2); ctx.lineTo(p.w / 3, p.h / 2);
          ctx.moveTo(0, -p.h / 2); ctx.lineTo(-p.w / 8, p.h / 2);
          ctx.stroke();
        } else {
          ctx.beginPath();
          ctx.moveTo(-10, -p.h / 2 + 3); ctx.lineTo(-5, p.h / 2 - 2);
          ctx.moveTo(10, -p.h / 2 + 2); ctx.lineTo(6, p.h / 2 - 3);
          ctx.stroke();
        }
        ctx.globalAlpha = 1;
    ...
    function drawDoodler(gs: GS) {
      ...
      bodyG.addColorStop(0, "#88f044"); bodyG.addColorStop(0.5, "#4acc18");
      bodyG.addColorStop(0.85, "#2e9a08"); bodyG.addColorStop(1, "#1a6004");
      ...
      ctx.beginPath();
      ctx.moveTo(-9, 6); ctx.quadraticCurveTo(0, 14, 9, 6); ctx.quadraticCurveTo(0, 10, -9, 6); ctx.fill();

      const wag = Math.sin(t * 0.22) * 2.5;
      ctx.fillStyle = "#ff3d88"; ctx.strokeStyle = "#cc1060"; ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(-3, 11); ctx.quadraticCurveTo(wag * 0.4, 14, 1, 20 + wag);
      ...
      eyeSocketL.addColorStop(0, "#88f044"); eyeSocketL.addColorStop(0.7, "#4acc18"); eyeSocketL.addColorStop(1, "#2a8808");
      ctx.fillStyle = eyeSocketL; ctx.strokeStyle = "#186004"; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(-9, -19, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke();

      const eyeSocketR = ctx.createRadialGradient(9, -19, 1, 9, -19, 11);
      eyeSocketR.addColorStop(0, "#88f044"); eyeSocketR.addColorStop(0.7, "#4acc18"); eyeSocketR.addColorStop(1, "#2a8808");
      ctx.fillStyle = eyeSocketR; ctx.strokeStyle = "#186004"; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(9, -19, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      ...
      const pDx = gs.pvx * 0.18;
      const pDy = -Math.abs(gs.pvy) * 0.08;
      ctx.fillStyle = "#111";
    ...
    function drawHUD(gs: GS) {
      ...
      ctx.fillText(`Best: ${gs.hi}`, 16, 40);

      if (gs.jetpack > 0) {
        ctx.fillStyle = "rgba(255,255,255,0.82)";
        ctx.beginPath(); ctx.roundRect(W - 110, 8, 102, 24, 8); ctx.fill();
      ...
      if (gs.hat > 0) {
        ctx.fillStyle = "rgba(255,255,255,0.82)";
        ctx.beginPath(); ctx.roundRect(W - 110, gs.jetpack > 0 ? 36 : 8, 102, 24, 8); ctx.fill();
    ...
    function drawMenu(gs: GS) {
      ...
      ctx.beginPath(); ctx.roundRect(W / 2 - 150, 100, 300, 220, 20); ctx.fill();
      ctx.strokeStyle = "#38c020"; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.roundRect(W / 2 - 150, 100, 300, 220, 20); ctx.stroke();
      ...
      ctx.fillText("Jump on platforms to go higher!", W / 2, 185);
      ctx.fillText("← → keys or tilt device", W / 2, 205);

      const grad = ctx.createLinearGradient(W / 2 - 85, 0, W / 2 + 85, 0);
      grad.addColorStop(0, "#56d830"); grad.addColorStop(1, "#28a010");
      ctx.fillStyle = grad;
      ctx.beginPath(); ctx.roundRect(W / 2 - 85, 220, 170, 48, 14); ctx.fill();
      ctx.fillStyle = "#fff";
      ctx.font = "bold 22px 'Comic Sans MS', cursive";
      ctx.fillText("▶  PLAY", W / 2, 252);

      if (gs.hi > 0) {
        ctx.fillStyle = "#a05010";
        ctx.font = "bold 14px 'Comic Sans MS', cursive";
        ctx.fillText(`Best: ${gs.hi}`, W / 2, 295);
      }
      ctx.restore();
    }
    ...
    function drawGameOver(gs: GS) {
      ...
      if (gs.score >= gs.hi && gs.score > 0) {
        ...
        ctx.fillText("🏆 New Best!", W / 2, 285);
      } else {
        ctx.fillStyle = "#888";
        ctx.font = "14px 'Comic Sans MS', cursive";
        ctx.fillText(`Best: ${gs.hi}`, W / 2, 285);
      }

      const grad = ctx.createLinearGradient(W / 2 - 80, 0, W / 2 + 80, 0);
      grad.addColorStop(0, "#56d830"); grad.addColorStop(1, "#28a010");
      ctx.fillStyle = grad;
      ctx.beginPath(); ctx.roundRect(W / 2 - 80, 305, 160, 48, 14); ctx.fill();
      ctx.fillStyle = "#fff";
    ...
    function update() {
      ...
      const left = gs.keys["ArrowLeft"] || gs.keys["KeyA"] || gs.tiltX < -0.15;
      const right = gs.keys["ArrowRight"] || gs.keys["KeyD"] || gs.tiltX > 0.15;
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (gs.phase === "menu") {
      if (mx > W / 2 - 85 && mx < W / 2 + 85 && my > 220 && my < 268) startGame();
  ...
  const handleCanvasTouch = useCallback((e: React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if (gs.phase !== "playing") {
      ...
      if (gs.phase === "menu" && mx > W / 2 - 85 && mx < W / 2 + 85 && my > 220 && my < 268) startGame();
      if (gs.phase === "dead" && mx > W / 2 - 80 && mx < W / 2 + 80 && my > 305 && my < 353) startGame();
    }
  }, [startGame]);

  // Mobile button handlers
  const pressLeft = useCallback((pressed: boolean) => {
    gsRef.current.keys["ArrowLeft"] = pressed;
    if (pressed) gsRef.current.keys["ArrowRight"] = false;
  }, []);

  const pressRight = useCallback((pressed: boolean) => {
    gsRef.current.keys["ArrowRight"] = pressed;
    if (pressed) gsRef.current.keys["ArrowLeft"] = false;
  }, []);
    ...
    >
      ...
      <div
        style={{
          ...
          flex: "0 1 auto",
          maxHeight: phase === "playing" ? "calc(100dvh - 120px)" : "92dvh",
      ...
      {/* Mobile controls — shown only during play */}
      {phase === "playing" && (
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          <button
            style={{
              ...
              height: "72px",
              borderRadius: "18px",
              background: "linear-gradient(135deg, #4acc18, #28900a)",
              border: "3px solid #186004",
              color: "#fff",
              fontSize: "32px",
              fontWeight: "bold",
              cursor: "pointer",
              boxShadow: "0 4px 16px rgba(0,0,0,0.4), 0 2px 0 #186004",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              userSelect: "none",
            ...
            onPointerDown={(e) => { e.preventDefault(); pressLeft(true); (e.currentTarget as HTMLElement).style.transform = "scale(0.93)"; }}
            onPointerUp={(e) => { e.preventDefault(); pressLeft(false); (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}
            onPointerLeave={(e) => { pressLeft(false); (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}
            onPointerCancel={(e) => { pressLeft(false); (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}
          >
            ◀
          </button>
          ...
          <button
            ...
            onPointerDown={(e) => { e.preventDefault(); pressRight(true); (e.currentTarget as HTMLElement).style.transform = "scale(0.93)"; }}
            onPointerUp={(e) => { e.preventDefault(); pressRight(false); (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}
            onPointerLeave={(e) => { pressRight(false); (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}
            onPointerCancel={(e) => { pressRight(false); (e.currentTarget as HTMLElement).style.transform = "scale(1)"; }}
          >
            ▶
          </button>
        </div>
